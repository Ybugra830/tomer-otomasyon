...
export default function PersonalDetails({ formData, handleInputChange, handleFileChange }) {
    return (
        <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 pt-4 border-t border-slate-200/60 mt-6">
                ...
                <FormGroup label="İndirim / Discount">
                    <div className="relative">
                        <select name="indirim" value={formData.indirim || ''} onChange={handleInputChange} className="form-input-premium appearance-none pr-10">
                            ...
                            {/* Burada veritabanından /api/indirimler/ endpointi uzerinden gelen options.indirimler map edilecek, şimdilik statik bir örnek bırakıyorum (ve App.jsx'te seçenekleri ayarlarız) */}
                            {formData.indirimOptions && formData.indirimOptions.map(opt => <option key={opt.id} value={opt.id}>{opt.ad}</option>)}
                        </select>
