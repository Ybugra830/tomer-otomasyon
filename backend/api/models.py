...
class Sube(models.Model):
    ...
    def __str__(self): return f"{self.ad} ({self.get_egitim_sekli_display()})"
...
class Aday(models.Model):
    ...
    kimlik_tipi = models.CharField(max_length=20, choices=KIMLIK_TIPI_CHOICES, blank=True, null=True)
    ad = models.CharField(max_length=100)
...
class BaseBasvuru(models.Model):
    DURUM_CHOICES = [
        ('BEKLIYOR', 'Bekliyor'),
        ('ONAYLANDI', 'Onaylandı'),
...
class SinavYuzYuze(BaseBasvuru):
    sinav_turu = models.CharField(max_length=150, blank=True, null=True)

class SinavCevrimIci(BaseBasvuru):
    sinav_turu = models.CharField(max_length=150, blank=True, null=True)
    vize_turu = models.CharField(max_length=100, blank=True, null=True)
    vize_baslangic = models.DateField(blank=True, null=True)
    vize_bitis = models.DateField(blank=True, null=True)
